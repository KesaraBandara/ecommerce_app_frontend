...
export default {
  ...
  methods: {
    async editProduct() {
      // Assuming your baseURL includes the version, adjust it accordingly
      axios.put(`http://localhost:8083/api/v1/product/update/${this.id}`, this.product)
          .then(() => {
            // Sending the event to the parent to handle
            this.$emit("fetchData");
            this.$router.push({ name: 'AdminProduct' });
            swal({
              text: "Product Updated Successfully!",
